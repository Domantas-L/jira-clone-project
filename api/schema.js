...
export const refresh_token = sqliteTable("refresh_token",
    ...
    expires: integer("Token_expires").notNull(),
...
export const ListTableRelation = relations(ListTables,
     ({ one, many }) => ({
    board: one(Boards, { fields: [ListTables.Board_id], references: [Boards.id] }),
    Tasks: many(Tasks),
}));
export const TasksRelation = relations(Tasks, 
    ({ one, many }) => ({
    list: one(ListTables, { fields: [Tasks.List_id], references: [ListTables.id] }),
    Task_tags: many(Task_tags),
}));

export const Tag_taskRelation = relations(Task_tags, 
    ({ one }) => ({
    task: one(Tasks, { fields: [Task_tags.Tasks_id], references: [Tasks.id] }),
    tag: one(Tags, { fields: [Task_tags.Tags_id], references: [Tags.id] }),
}));

export const User_BoardsRelation = relations(Users,({many})=> ({Boards: many(Boards)}));